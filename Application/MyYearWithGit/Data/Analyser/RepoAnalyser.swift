...
class RepoAnalyser {
    ...
    func analysis(at: URL, session: UUID) {
        ...
        guard let currentCommitLogs = grabGitCommitLog() else {
            debugPrint("failed to code git log, giving up!")
            return
        }
        ...
        for currentCommitLog in currentCommitLogs {
            ...
            autoreleasepool {
                ...
                queue.async {
                    debugPrint("calling analysis on commit \(currentCommitLog.hash) with \(currentCommitLog.autherEmail)")
        ...
        guard commitResults.count > 0 else {
            debugPrint("no data was generated from repo \(at.path), skipping submit")
            return
        }
        debugPrint("analysis completed, submitting \(commitResults.count) result to repo")
        let repoResult = GitRepoResult(commits: commitResults)
        currentResults.repos.append(repoResult)
    }

    func commitResult() -> ResultPackage {
        debugPrint("compiling result for \(currentResults.repos.count) repos")
